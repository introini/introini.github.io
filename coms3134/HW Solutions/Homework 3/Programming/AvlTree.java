...
public class AvlTree<AnyType extends Comparable<? super AnyType>> implements Iterable<AnyType> {
  ...
  public AnyType get(AnyType x) {
    AvlNode<AnyType> node = get(x, root);
  ...
  private AvlNode<AnyType> get(AnyType x, AvlNode<AnyType> t) {
    while (t != null) {
      int compareResult = x.compareTo(t.element);
  ...
  /**
  * The methods below are for Problem 2
  * They will NOT work if the tree is not initialized
  * with WordLinePair as its type.
  */

  public void indexWord(String word, int line) {
    WordLinePair pair = new WordLinePair(word);
    WordLinePair existingPair = (WordLinePair) this.get((AnyType) pair);
    if(existingPair == null) {
      pair.line.add(line);
      this.insert((AnyType) pair);
    }
    else {
      existingPair.line.add(line);
    }
  }

  public List getLinesForWord(String word) {
    WordLinePair pair = (WordLinePair) this.get((AnyType) new WordLinePair(word));
    return pair.line;
  }

  public void printIndex() {
    printIndex((AvlNode<WordLinePair>) root);
  }

  private void printIndex(AvlNode<WordLinePair> node) {
    if(node == null)
      return;
    printIndex(node.right);
    printIndex(node.left);
    System.out.println("Word: " + node.element.word + " Lines: " + node.element.line);
  }
  ...
  public class AvlTreeIterator<T> implements Iterator<T> {
    ...
    private void infixTraversal(AvlNode<T> node) {
      if (node.left != null) {
