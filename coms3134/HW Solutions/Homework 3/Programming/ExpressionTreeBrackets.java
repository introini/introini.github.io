...
public class ExpressionTreeBrackets {

  private String expression;
  private ExpressionNode expressionTree;

  public ExpressionTreeBrackets(String expression) {
  ...
  private void buildTree() {
    ...
    for (String token : tokens) {
      if (isNumeric(token)) {
        ...
        if (!left.isNumber()) {
          if (node.mdas > left.mdas) {
            left.requiresBrackets = true;
          }
        }

        if (!right.isNumber()) {
          if (node.mdas > right.mdas) {
            right.requiresBrackets = true;
          }
        }
  ...
  private void infix(ExpressionNode node, StringBuilder sb) {
    if (!node.isNumber() && node.requiresBrackets) {
      sb.append("( ");
    }
    if (node.left != null) {
      infix(node.left, sb);
  ...
  public static void main(String[] args) {
    ExpressionTreeBrackets tree = new ExpressionTreeBrackets("4 3 - 9 3 / / 2 8 / 3 6 + - *");
